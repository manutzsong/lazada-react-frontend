...
export default class App extends React.Component {
			...
			transactions.forEach(async (x, xIndex) => {
				...
				if (typeof transactionsAppend[groupThis] === "undefined") {//if first
					...
					if (!orderFiltered) {//if undefine probably from pulling only Delivered but forgot other status; this wil lcatch all undefined
						...
						orderFiltered = resultReturned.data.data;
					}
	...
	countFrequency = () => {
		new Promise((resolve) => {
			...
			this.state.resultOrders.forEach(i => {
				// countFlattenTransaction[ i.shop_sku.split("_")[0] ].count = countFlattenTransaction[ i.shop_sku.split("_")[0] || 0 +=1
				let matchThis = i.address_billing.post_code;
				if (typeof countsCity[matchThis] === "undefined") {
					countsCity[matchThis] = { count: 1, product: i, title: matchThis };
				} else {
					countsCity[matchThis].count =
						(countsCity[matchThis].count || 1) + 1;
				}
			});

			let orderedCountsCity = [];
			for (let z in countsCity) {
				countsCity[z].postalCode = this.state.postalCode.find(h => h["zip"] === countsCity[z].title);
			...
			// }
			// for (let z in countsSKUs) {
			// 	pieThisCity.labels.push(countsSKUs[z].title);
			// 	pieThisCity.values.push(countsSKUs[z].count);
			...
			<div className="">
				<div className="d-flex align-items-center flex-md-row flex-column m-0">
					...
					<div className="d-flex justify-content-between flex-column">
						<div>
							<h3>Milestone :</h3>
							<h6>- Order Count <span role="img" aria-label="heart">💙</span></h6>
							<h6>- Item Count <span role="img" aria-label="heart">💙</span></h6>
							<h6>- Python SV still use for /getorderitems <span role="img" aria-label="heart">💙</span></h6>
						</div>
						<Divider />
						<h6>Contact <span role="img" aria-label="heart">🍺</span> : </h6>
	...
	updateBlock = () => {
		return (<div>
			...
			<h5>Help me by buying from this store อุดหนุนร้านคนทำ <a href="https://www.lazada.co.th/shop/sn-fasion/">SN-Fashion <span aria-label="heart" role="img">💗</span></a></h5>
			...
			return <div>
				<div className="row py-3">
					...
					<div className="col-md-3 col-12 order-md-1 order-0">
						...
						<List disablePadding dense className="box-shadow">
							...
							<ListText primary={"฿ " + this.state.wideFinancial.shippingFeeVoucher} secondary="Shipping Fee Voucher" divider={true} />
							<ListText primary={"฿ " + this.state.wideFinancial.adjustmentShippingFee} secondary="Adjustment Shipping Fee" divider={true} />
							<ListText primary={"฿ " + this.state.wideFinancial.otherFee} secondary="Other Fee" divider={true} />
						</List>
					</div>
				</div>
	...
	render() {
		...
		return (
			...
			<div className="bg-light p-3">
				...
				<div className="row">
					<div className="col-md-6 col-12"><TopProductPie className="" resultData={this.state.chartTopSellArray} title="Top Selling By Product" /></div>
					<div className="col-md-6 col-12"><TopProductPie className="" resultData={this.state.chartAllSKUsArray} title="Top Selling By SKU" /></div>
				</div>
